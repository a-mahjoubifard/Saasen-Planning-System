...
/**
 * REST controller for managing {@link com.saasen.nl.domain.Resource}.
 */
@RestController
...
public class ResourceResource {
    ...
    public ResponseEntity<Resource> createResource(@Valid @RequestBody Resource resource) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/resources/" + resource.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, resource.getId().toString()))
            .body(resource);
    }
    ...
    public ResponseEntity<Resource> partialUpdateResource(
        ...
        Optional<Resource> result = resourceRepository
            ...
            .map(existingResource -> {
                updateIfPresent(existingResource::setName, resource.getName());
                updateIfPresent(existingResource::setResourceType, resource.getResourceType());
                updateIfPresent(existingResource::setStatus, resource.getStatus());
